...
from stock_price_prediction.data_sources import chart, quote_summary, moving_average, percent_change, fmt_number, fmt_percent, raw_value, unix_to_iso, MarketDataError
from stock_price_prediction.forecast_model import build_quant_forecast, build_multi_horizon_forecast
from stock_price_prediction.ir_scraper import get_ir_data
from stock_price_prediction.news_analyzer import get_geopolitical_analysis
from stock_price_prediction.company_master import resolve_from_master
...
def collect_all_data(company_name: str) -> dict:
    ...
    print("[1/5] Fetching price trend data...")
    try:
        ...
        price_data = {
            ...
            "return_1m": f"{change_1m*100:.2f}%" if change_1m else None,
            "return_3m": f"{change_3m*100:.2f}%" if change_3m else None,
            "return_6m": f"{change_6m*100:.2f}%" if change_6m else None,
            "return_1y": f"{change_1y*100:.2f}%" if change_1y else None,
        ...
        print(f"  Price: {price_data['latest_price']} | 1M: {price_data['return_1m']} | 3M: {price_data['return_3m']}")
    except MarketDataError as e:
        price_data = {"error": str(e)}
        print(f"  Error: {e}")

    print("[2/5] Fetching financial data...")
    try:
        ...
        print(f"  P/E: {fin_data['trailing_pe']} | Fwd P/E: {fin_data['forward_pe']} | Margin: {fin_data['operating_margin']}")
    except MarketDataError as e:
        fin_data = {"error": str(e)}
        print(f"  Error: {e}")

    print("[3/5] Fetching macro data...")
    macro_tickers = {
        "usd_jpy": "JPY=X",
        "us_10y_yield": "^TNX",
        "wti_crude_oil": "CL=F",
    ...
    for name, symbol in macro_tickers.items():
        try:
            ...
            macro_data[name] = {
                ...
                "return_1m": f"{ret_1m*100:.2f}%" if ret_1m else None,
                "return_6m": f"{ret_6m*100:.2f}%" if ret_6m else None,
            }
            print(f"  {name}: {macro_data[name]['latest']} | 1M: {macro_data[name]['return_1m']}")
        except MarketDataError as e:
            macro_data[name] = {"error": str(e)}

    print("[4/5] Fetching IR data...")
    ...
    ir_summary = {
        ...
        "financial_highlights": ir_data.get("financial_highlights", {}),
    }
    print(f"  IR URL: {ir_summary['ir_url']}")
    print(f"  Documents: {ir_summary['documents_count']}")

    print("[5/5] Fetching geopolitical news...")
    geo_data = get_geopolitical_analysis()
    print(f"  Risk level: {geo_data.get('risk_level', 'unknown')}")
    print(f"  Key events: {len(geo_data.get('key_events', []))}")

    print("\n[6/6] Building forecasts...")
    try:
        quant_forecast = build_quant_forecast(ticker)
        print(f"  Current price: {quant_forecast.get('current_price')}")
        print(f"  Forecast (3M): {quant_forecast.get('forecast_price_base')}")
        print(f"  Expected return: {quant_forecast.get('expected_return_pct')}%")
    ...
    try:
        ...
        for f in multi_forecast.get("direction_forecasts", []):
            print(f"  {f['label']}: {f['direction']} ({f['direction_score']*100:.0f}%)")
    except Exception as e:
...
def main():
    ...
    if args.json:
        ...
        print("\n" + "=" * 60)
        print("Data collection complete. Use --json for full JSON output.")
        print("=" * 60)
